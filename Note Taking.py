...
db=pymysql.connect("localhost",'root','yogesh@123456','deepa')
cursor=db.cursor()
...
def add():
    ...
    def save():
        ...
        cursor.execute("insert into deepa values('" + sub+ "','" + notes + "');")
        db.commit()
        list.append(sub)
        update()


    label = Label(add_root, text="Subject", bg="skyblue", font="bold 15")
    ...
    save_button = Button(add_root, text="Save", font="bold 13", bg="red", fg="white", command=save)
...
def show_all():
    ...
    qr=cursor.execute("select subject from deepa;")
    a=cursor.fetchall()
    for i in a:
        for j in i:
            # show_list=Label(show_root,text=j)
            # show_list.place(x=20,y=20)
            print(j)
